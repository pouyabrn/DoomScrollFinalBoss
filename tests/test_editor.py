...
def test_deterministic_editorial_is_grounded(make_story: Callable[..., Story]) -> None:
    ...
    assert result.linkedin_draft.evidence_item_ids == [stories[0].id]
    assert "scan" not in " ".join(result.linkedin_draft.post_lines).casefold()
...
def test_editor_rejects_unknown_and_duplicate_ids(make_story: Callable[..., Story]) -> None:
    ...
    ungrounded_linkedin = valid.model_copy(
        update={
            "linkedin_draft": valid.linkedin_draft.model_copy(
                update={"evidence_item_ids": ["invented"]}
            )
        }
    )
    with pytest.raises(ValueError, match="LinkedIn evidence"):
        OpenRouterEditor._validate_ids(ungrounded_linkedin, {story.id})
...
async def test_openrouter_editor_uses_strict_grounded_output(
    ...
    assert "impression_potential" not in linkedin_schema["properties"]
    assert "model_confidence" not in linkedin_schema["properties"]
